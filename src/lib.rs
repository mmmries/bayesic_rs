...
impl Bayesic {
  ...
  pub fn classify(& self, tokens: Vec<String>) -> HashMap<String, f64> {
    ...
    let prior: f64 = 1.0 / num_classes;
    for class in &self.classifications {
      let mut p_klass = prior;
      for token in &tokens {
        if self.by_token.contains_key(token) && self.by_token[token].contains(class) {
...
mod tests {
  ...
  fn empty_classification() {
    ...
    assert_eq!(classification["jojo"], 0.2);
  }
  ...
  fn key_word_classification() {
    ...
    assert!(classification["jurassic_park"] < 0.25);
  }
  ...
  fn generic_word_classification() {
    ...
    // two of our movies don't have it so they should just have the prior probability
    assert_eq!(classification["jurassic_park_ii"], 0.2);
    assert_eq!(classification["jurassic_park_iii"], 0.2);
  }
